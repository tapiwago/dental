...
export default function Users() {
	...
	const filterUsers = () => {
		...
		if (activeFilter !== 'all') {
			filtered = filtered.filter((user) => (activeFilter === 'active' ? user.isActive : !user.isActive));
		}
	...
	return (
		<Box sx={{ p: 3 }}>
						...
						<TableBody>
							{paginatedUsers.map((user) => (
								...
								>
									...
									<TableCell>
										<Typography variant="body2">{user.department || 'Not assigned'}</Typography>
									</TableCell>
									<TableCell>
										<Chip
			...
			>
				...
				<DialogContent>
					<DialogContentText>
						Are you sure you want to deactivate user "{userToDelete ? getFullName(userToDelete) : ''}"? This
						action will make the user unable to access the system but will preserve their data.
					</DialogContentText>
				</DialogContent>
				<DialogActions>
